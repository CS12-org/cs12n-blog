...
function VerifyEmail() {
  ...
  const [expirationTimer, setExpirationTimer] = useState(0); // Simulate verification API call
  const [email, setEmail] = useState('');
  const verifyEmail = async () => {
    try {
      const response = await axios.get(`/api/auth/verify-email?token=${token}`);
      setStatus('success');
      //setExpirationTimer(5);
  ...
  const resendEmail = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'ارسال مجدد ایمیل با خطا مواجه شد');
    }
  ...
  console.log(status, expirationTimer, 'status');
  ...
  return (
    <main className='min-h-dvh flex flex-col items-center px-6'>
      <form className='relative flex flex-col items-stretch my-auto bg-crust rounded-2xl p-5 w-full max-w-sm'>
        ...
        {status === 'success' && (
          <p className='text-center text-label-md text-green my-1'>ایمیل شما با موفقیت تایید شد!</p>
        )}
        {status === 'success' && (
