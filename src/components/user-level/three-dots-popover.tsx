...
export default function ThreeDotPopover() {
  ...
  const ThreeDottsButton = forwardRef<HTMLButtonElement, { onClick: () => void }>(
    ({ onClick }, ref) => (
      <button
        ref={ref}
        onClick={onClick}
    ...
    <div className="relative inline-block">
      ...
      {isVisible && (
        <section
          ...
          role="menu"
        >
         <button className="flex gap-[7px]"><Repead className="w-[20px] h-[20px] " fill="#A5ADCB"/> از سرگیری دوره </button>
         <button className="flex gap-[7px] items-center"> <Trash className="w-[25px] h-[25px] stroke-subtext-0 font-bold"/>پاک کردن دوره </button>
         <button className="flex gap-[7px]"><Survey /> نظرسنجی</button>
